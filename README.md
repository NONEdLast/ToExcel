...
在实际工作中，我们经常需要将结构化文本或JSON数据转换为Excel表格进行进一步分析和处理。传统的转换方式往往需要手动处理，耗时且容易出错。本工具的出现旨在解决这一痛点，提供自动化、智能化的数据转换方案。
...
- **内置运行时**：包含嵌入式Python运行时，无需系统级Python安装
...
1. **获取项目**：从项目仓库或分发渠道获取完整项目文件
2. **直接运行**：
   - **Windows**：双击运行 `run_gradio.bat` 批处理文件
   - **Linux/macOS**：运行 `run_gradio.sh` 脚本（需要根据系统调整）

3. **首次运行**：
   - 首次运行时会初始化嵌入式Python环境
   - 如果依赖库未预安装，会自动下载安装

4. **访问界面**：
   - 应用启动后，在浏览器中打开终端显示的URL（通常为 `http://localhost:7861`）
...
├── run_gradio.bat          # Windows Web界面运行脚本
├── run_gradio.sh           # Linux/macOS Web界面运行脚本
├── runtime/                # 嵌入式Python运行时
├── lib/                    # 本地依赖库目录
└── README.md               # 项目说明文档
```
...
### 3.27 update
- 新增**嵌入式Python运行时**，无需系统级Python安装
- 添加`runtime`文件夹，包含Python 3.11.6嵌入式版本
- 更新`run_gradio.bat`批处理文件，使用嵌入式Python运行应用
- 修改`gradio_app.py`，确保能正确找到项目模块
